...
CostSentinel uses **PostgreSQL with TimescaleDB** for durable request history storage, replacing the original Firestore design. This provides:
...
TimescaleDB automatically deletes old data:
...
-- Expected: 90 days (7776000 seconds)
```
...
```sql
-- Check oldest record
SELECT MIN(timestamp) as oldest_record 
...
```sql
-- Count records expiring in next 24 hours
SELECT COUNT(*) 
FROM request_history 
...
-- Check retention policy
SELECT policy_name, config_interval 
